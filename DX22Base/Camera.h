...
private:
	// ===メンバ変数宣言=====
	TDiType<float> m_fRadianVelocityWeak;	//蓄積角速度：弱(ラジアン角)	x:縦, y:横
	TDiType<float> m_fRadianVelocityStrong;	//蓄積角速度：強(ラジアン角)	x:縦, y:横
	TDiType<float> m_fAddRadianWeak;		//角速度増加量：弱(ラジアン角)	x:縦, y:横
	TDiType<float> m_fAddRadianStrong;		//角速度増加量：強(ラジアン角)	x:縦, y:横
