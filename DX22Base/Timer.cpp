...
const int TIME_LEFT_TIRTHY_SEC = 30 * 60;									//時間残り30秒
const int TIME_LEFT_TEN_SEC = 10 * 60;										//時間残り10秒
const float TIME_LEFT_SCALE_TIRTHY = 0.005;		// 残り30秒拡大縮小の比率
const float TIME_LEFT_SCALE_TEN = 0.02f;		// 残り10秒拡大縮小の比率(少し大きく)
const int TIME_SCALE_SPEED_TIRTHY = 100;			// 拡大縮小一回のフレーム数(100flame一回遅い)
const int  TIME_SCALE_SPEED_TEN = 30;		// 拡大縮小一回のフレーム数(30flame一回速い)
...
CTimer::CTimer()
	...
	, m_fTimerScale(1.0f)		// 時間普段のスケール倍率
	, m_nTimerScaleCnt(0)
{
	//数字のテクスチャ読む込み
...
{
	m_nTimerScaleCnt++;									// タイマーフレームカウント加算
	if (m_nTimerScaleCnt % nFlame <= (nFlame / 2) - 1)	//設定したフレーム数の前半加算(拡大)、後半減算(縮小)
	{
		m_fTimerScale += fSize;
	}
	else
...
/* ========================================
	...
	戻値：経過時間
=========================================== */
int CTimer::GetErapsedTime()
{
	// =============== 提供 ===================
	return (STAGE_TIME - m_nTimeCnt);	//経過時間
}