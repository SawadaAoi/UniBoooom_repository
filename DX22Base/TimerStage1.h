...
#ifndef __TIMER_STAGE_1_H__	//TimerStage1.hインクルードガード
...
public:
	// ===プロトタイプ宣言===
	CTimerStage1();	//コンストラクタ
private:
	// ===定数定義 ==========
	const float SLM_PARAM_CHANGE_TIME[CTimer::TIME_STATE::STATE_MAX] = { 60.0f, 120.0f, 180.0f };	// 経過時間の秒数
	const int	SLM_CREATE_NUM[STATE_MAX] = { 10, 15, 20 };											// 最大生成数(25はMAX_SLIME_NUM以下の値)
	const float SLM_CREATE_INTERVAL_TIME[STATE_MAX] = { 1.0f, 1.0f, 1.0f };							// 生成間隔
