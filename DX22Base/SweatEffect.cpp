...
const float SWEAT_EFFECT_SIZE = 0.7f;			// エフェクトサイズ
...
/* ========================================
	...
	戻値：なし
=========================================== */
CSweatEffect::CSweatEffect(TPos3d<float> fPos, TPos3d<float> fRadian, float fTime, Effekseer::EffectRef sweatEffect, const CCamera* pCamera)
	: m_pCamera(nullptr)
	, m_fEffectTime(fTime)
	, m_bDelFlg(false)
	, m_nDelFrame(0)
{
	...
	m_efcSweatHnadle = LibEffekseer::GetManager()->Play(m_SweatEffect, m_Transform.fPos.x, m_Transform.fPos.y , m_Transform.fPos.z);	// エフェクトの開始
	LibEffekseer::GetManager()->SetScale(m_efcSweatHnadle, SWEAT_EFFECT_SIZE, SWEAT_EFFECT_SIZE , SWEAT_EFFECT_SIZE);	// エフェクトのサイズを設定
	LibEffekseer::GetManager()->SetRotation(m_efcSweatHnadle, m_Transform.fRadian.x + 3.0f, 90.0f, m_Transform.fRadian.z);		// Y軸90度で固定する
...
{
	m_Transform.fPos = fPos;
	LibEffekseer::GetManager()->SetLocation(m_efcSweatHnadle, m_Transform.fPos.x, m_Transform.fPos.y + 5.5f, m_Transform.fPos.z);
	DisplayTimeAdd();
}
...
/* ========================================
	...
	戻値：なし
======================================== */
void CSweatEffect::SetCamera(CCamera * pCamera)
{
	m_pCamera = pCamera;
}
