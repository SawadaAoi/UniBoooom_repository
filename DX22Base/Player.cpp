...
const float PLAY_SE_CNT = 20;								// チャージ中のSEが再生されるまでのカウント
const int ADJUST_SWING_WAIT = 13;							// ハンマーのアニメーションと実際の当たり判定とのずれを修正するための時間の割合
	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				{
					// チャージのカウントをリセットしてプレイヤーの状態をチャージ状態に移行
					m_fChargeCnt = 0;
